...
[Grunt](http://gruntjs.com/ "Grunt"), так что это было в большей мере
руководство для
...
виде Grunt задания, вам нужно установить Ruby ([полная инструкция по установке
...
сообщает вам сколько времени занимает каждая задача и общее время билда.
3. `load-grunt-config`: позволяет вам содержать ваш основной Gruntfile коротким
и опрятным. Подробнее об этом чуть позже.
...
Он позволяет нам устанавливать конфигурацию для каждого из наших заданий в
отдельные файлы — это удобнее, чем просто складывать всё в один длинный
Gruntfile.
...
```
Если для примера взять dev-задачи, вы увидите, что в начале должен запуститься  
...
Управлять `grunt-contrib-clean` несложно: здесь я просто удаляю содержимое директории `dist/`. Используйте эту задачу с осторожностью - она будет удалять без разбора всё, что вы ей скажете — без всяких оповещений, поэтому убедитесь, что всё правильно настроили.
...
`nested` и активировал sourceMap.
