...
[Grunt](http://gruntjs.com/ "Grunt"), так что это было в большей мере руководство для
меня самого, чем для кого-то другого. Теперь, спустя 18 месяцев, я чувствую
необходимость пересмотреть свои способы использования Grunt, потому что за
...
виде Grunt задания, вам нужно установить Ruby ([полная инструкция по установке здесь](https://www.ruby-lang.org/en/installation/)) и, после того, как вы сделаете это, [Sass](http://sass-lang.com/download.html) gem:
...
Для начала нашему проекту нужна пара директорий, их структура отражена ниже:
...
Во-первых, я больше не использую ‘скаффолд’ инструменты (такие как `grunt init`
или [Yeoman](http://yeoman.io/)). Я устанавливаю всё самостоятельно, а это
значит, что я гораздо лучше понимаю что происходит. На самом деле  всё не так
...
Давайте пойдем дальше и создадим наш базовый `package.json` файл. Этот файл
через короткое время будет содержать в себе зависимости нашего проекта.
Добавьте следующее (очевидно, что нужно сменить ссылку с ‘my project’ на
действительное имя вашего проекта):
...
Запустите по очереди каждую строчку из кода, приведенного ниже, одну за другой:
...
Вот описание того, что мы только что установили:
...
3. `load-grunt-config`: Позволяет вам держать ваш основной Gruntfile в коротком и упорядоченном виде. Подробнее об этом чуть позже.
4. `grunt-concurrent`: Запускает задачи одновременно - Grunt из-коробки будет запускать каждую задачу одну за другой, что может затянуться на время, в зависимости на количестве и типе заданий, необходимых для выполнения. Однако, часто есть задания, которые не зависят от других заданий, которые могут быть запущены в одно и то же время.
...
10. `jshint-stylish`: Полностью опционально, но это преобразовывает вывод `grunt-contrib-jshint` в отличный вид.
...
Конфигурирование `grunt-contrib-clean` - это очень просто. Здесь я просто удаляю содержимое директории `dist/`. Используйте эту задачу с осторожностью - она будет удалять без разбора всё, что вы ей скажете без всяких оповещений, поэтому убедитесь, что сконфигурировали её правильно.
...
Если вы закончили установку вашего проекта, как приведено выше, тогда вы можете запустить задачи. Как обсуждалось ранее, есть различные псевдонимы задач, которые вы можете запускать. Сейчас просто запустите `grunt` из командной строки из корня вашего проекта.
...
Мне нравится этот небольшой вывод, который выдаёт `time-grunt`. Я могу увидеть как долго каждая одновременная задача выполняется, плюс как много времени занимает весь процесс - круто!
...
Ну вот, пожалуй, и всё по этому вопросу. Если вы экспериментируете с вышеприведённым, то скоро вы набьёте руку, начнёте добавлять новые задачи и подстраивать под себя рабочий процесс.
